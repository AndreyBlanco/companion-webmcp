...
| Persistir `rawText` antes de procesamiento semántico | La extracción ocurre en `interpret()` antes de `store.save()` | Lab B escribe `00-input-records.json` antes del dry ingestion, pero no es la persistencia de la app | MISSING en producto; MOCKED en Lab B | `src/core/semantic-memory.js:46-60`; `src/lab-b/index.js:134-144`. Contradice INV-02 en la ruta pública |
...
| Confirmación explícita antes de memoria semántica | Draft separado, token single-use y `confirmed: true` obligatorio | Cubierto también por experimentos | IMPLEMENTED, con reserva raw-first | `src/core/semantic-memory.js:52-60`; la memoria se guarda tras confirmar, aunque el Builder ya corrió |
