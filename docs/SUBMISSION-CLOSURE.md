...
The competition demo starts empty and accepts synthetic observations up to 1,000 characters, a conservative allowance for roughly 30 seconds of speech. A protected Netlify function first proposes only the subject. The person corrects or approves the text and subject; Companion persists both before a second call builds the graph. Graph processing uses minimal reasoning effort and a 45-second internal timeout. Deterministic validation requires supported types, the human-confirmed subject and literal source excerpts. Memory remains browser-session-local, retrieval is deterministic by subject, and the WebMCP payload contains evidence rather than a generated answer.
