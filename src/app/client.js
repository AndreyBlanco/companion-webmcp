...
async function prepare(rawText) {
  ...
  try { draft = await capabilities.prepare({ rawText }); $('raw-text').value = rawText; await populateSubjectChoices(draft.subjectResolution.subject); $('resolution').textContent = `${draft.subjectResolution.status}: ${draft.subjectResolution.reason}`; show(draft); step('review'); status('Choose an existing subject or approve/edit the proposal before saving.', 'success'); }
...
registerWebMcp(document.modelContext, capabilities).then((result) => { $('webmcp-status').textContent = result.available ? `WebMCP available: ${result.toolName}` : 'WebMCP is unavailable in this browser; you can still capture entries.'; });
